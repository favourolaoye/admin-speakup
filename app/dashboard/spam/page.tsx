...
export default function Page() {
  const setspamData = useAuthStore((state) => state.setspamData);
  const spamdata = useAuthStore((state) => state.spamdata) as Report[];
  ...
  const spamReports = data?.filter((report) => report.category === 'spam') || [];
  ...
  useEffect(() => {
    if (spamdata && spamdata.length > 0 && !selectedReport) {
      setSelectedReport(spamdata[0]);
    }
  }, [spamdata, selectedReport]);
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        ...
        <div className="flex space-x-4 mt-6 border-t pt-4">
          <button
            ...
            onClick={() => selectedReport && handleDelete(selectedReport._id)}
          >
